...
class ProductionTradingSystem:
    ...
    def run_full_analysis(self):
        ...
        # 8. レポート保存
        report_files = self.report_generator.save_reports_to_files(
            buy_report, portfolio_report, performance_report
        )
        # 3つの戻り値の場合と2つの戻り値の場合に対応
        if isinstance(report_files, tuple) and len(report_files) == 3:
            markdown_file, text_file, json_file = report_files
        else:
            json_file, text_file = report_files
        ...
        logger.info(f"📁 詳細レポート: {text_file}")
